...
void Qt_DX11_Window::AddDockWindow()
{
	...
	mPropertyDock->setWidget(mPropertyBrowser);
	addDockWidget(static_cast<Qt::DockWidgetArea>(1), mPropertyDock);

	mPropertyDock->raise();
}

void Qt_DX11_Window::CreatePropertyGrid()
{
	...
	mPropertyBrowser->setFactoryForManager(variantManager, variantFactory);
	mPropertyBrowser->addProperty(topItem);
	mPropertyBrowser->setPropertiesWithoutValueMarked(true);
	mPropertyBrowser->setRootIsDecorated(false);
	mPropertyBrowser->show();
}
